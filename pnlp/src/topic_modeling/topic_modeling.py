...
# tuning class
from pnlp.src.topic_modeling.topic_modeling_utils.grid_search import HyperparameterTuning
# data bass class
from pnlp.src.topic_modeling.topic_modeling_utils.mongo_accessor import MongoAccessor
...
def get_fasttext_embeddings(data: pd.DataFrame) -> pd.Series:
    ...
    model = fasttext.load_model(f'{pnlp_path}\\crawl-300d-2M-subword.bin')
...
def model_topics(data, embeddings, cluster_algorithm, normalization, dim_reduction, outliers, run_grid_search):
    ...
    try:
        ...
        if embeddings == "fasttext":
            embedding_file = f"{pnlp_path}/src/topic_modeling/topic_modeling_embeddings/_mean_embeddings_fasttext"
        elif embeddings == "bert":
            embedding_file = f"{pnlp_path}/src/topic_modeling/topic_modeling_embeddings/_mean_embeddings_bert"
        ...
        if embeddings == 'fasttext':
            ...
            with open(f"{pnlp_path}/src/topic_modeling/topic_modeling_embeddings/_mean_embeddings_fasttext", "wb") as fp:
            ...
            with open(f"{pnlp_path}/src/topic_modeling/topic_modeling_embeddings/_mean_embeddings_bert", mode="wb") as file_handle:
